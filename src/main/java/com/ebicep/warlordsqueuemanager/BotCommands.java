...
public class BotCommands implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String s, String[] args) {
        ...
        Optional<TextChannel> botTeams = WarlordsBotManager.getTextChannelCompsByName("bot-teams");
        Optional<TextChannel> gsTeams = WarlordsBotManager.getTextChannelCompsByName("gs-teams");
        ...
        switch (input.toLowerCase()) {
            ...
            case "inputexperimental":
                ...
                if (args.length == 1) {
                    WarlordsBotManager.getTextChannelCompsByName("games-backlog").ifPresent(textChannel -> textChannel.sendMessage("/" + input + " " + DatabaseManager.lastWarlordsPlusString).queue());
                ...
                WarlordsBotManager.getTextChannelCompsByName("games-backlog").ifPresent(textChannel -> textChannel.sendMessage("/" + input + " " + DatabaseManager.lastWarlordsPlusString + " " + args[1]).queue());
                return true;
        }
        return true;
    }
